...
public class BaseController {
	...
	public String reLogin = "/suser/jsp/login/loginOut";
	public String loginError = "login!loginInput.do?rdtType=1";
	public String limitVerifyError = "/suser/jsp/login/limitVerifyError.jsp";
	 ...
	 * 将错误信息放到页面中,后台用
	 */
	public String setExceptionRequestAdmin(HttpServletRequest request,
			Exception e) {
		...
		if (!ObjectUtils.isNull(messages)) {
			...
			for (int i = 0; i < messages.length; i++) {
				/*
				 * buffer.append(messages[i].getClassName());
				 * buffer.append(messages[i].getMethodName()).append("(");
				 * buffer.append(messages[i].getFileName()).append(":");
				 * buffer.append(messages[i].getLineNumber()).append(")");
				 */
